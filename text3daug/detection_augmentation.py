...
from .visualize import bbox3d_to_o3d, o3d_visualize_mesh, o3d_visualize_pc


class DetectionInstances(object):
    def __init__(
        self,
        scanner_config: Dict,
        instance_dict: Dict,
    ...
    def instance_aug(
        self, point_xyz, point_feat=None
    ) -> Tuple[np.ndarray, np.ndarray, List[int], np.ndarray]:
        """
        Inserts instances from the .pkl file and return new bbox labels.
        NOTE: ! Only new bounding boxes are returned, and you have to add them
        to your ground truth.

        Args:
            point_xyz: [N, 3], point location
            class_names: [N], List of string class names in your dataset format.
            point_feat: [N, 1], Remission

        Returns:
            point_xyz: [N, 3] Updated pointcloud with instances
            box_labels: [add_num, 7] New bounding boxes, with (add_num, [x, y, z, dx, dy, dz, heading]).
            label_ints: [add_num] List of integers corresponding to the class in the .pkl file.
                NOTE: These integers have to be converted to strings depending on the dataset.
            point_feat: [N, 1] Updated Remission. Will not be returned if point_feat=None.
        """
        if self.visualize:
        ...
        label_ints = []
        box_labels = []
        # Keep track of the classes of each added bbox
        ...
        for n, count in zip(class_inst, class_inst_count):
            ...
            for idx in instance_choices:
                ...
                for _ in range(5):
                    ...
                    instance_xyz_init, heading = self.random_z_rotate_and_xy_translate(
                        instance_xyz
                    )
                    ...
                    instance_polar_placed, heading_offset = self.search_empty_space(
                        point_polar,
                        instance_polar_init,
                        heading,
                        self.min_range,
                ...
                if point_feat is not None:
                    ...
                    filtered_feat = expand_array_2(filtered_feat)
                ...
                # create bounding box of added instance
                instance_bbox = self.create_3d_bounding_box(
                    instance_lidar, heading_offset, instance_xyz
                )
                box_labels.append(instance_bbox)
                label_ints.append(self.instance_list[n])

        box_labels = np.concatenate(box_labels, axis=0)

        if self.visualize:

            N = box_labels.shape[0]
            o3d_bboxes = [
                bbox3d_to_o3d(box_labels[l, :], rgb=[0, 1, 0]) for l in range(N)
            ]

            o3d_visualize_pc(point_xyz[:, :3], o3d_bboxes)

        # Return data
        if point_feat is not None:
            return point_xyz, box_labels, label_ints, point_feat
        else:
            return point_xyz, box_labels, label_ints
    ...
    def search_empty_space(
        ...
        Args:
            ...
            heading (float): Z-axis rotation of the instance.
        ...
        return inst_polar, heading_offset
    ...
    def remove_occluded_points(
        ...
        return filtered_point_xyz, filtered_feat

    def create_3d_bounding_box(
        self, instance_points, heading_angle, instance_points_original
    ):
        # calculate extent of the input instance at the beginning
        extent_x = np.max(instance_points_original[:, 0]) - np.min(
            instance_points_original[:, 0]
        )
        extent_y = np.max(instance_points_original[:, 1]) - np.min(
            instance_points_original[:, 1]
        )

        # check, if correction is necessary
        if extent_y > extent_x:
            # rotate instance with 90 degrees
            corr_heading_angle = heading_angle + (np.pi / 2) + (np.pi)
        else:
            # no changes
            corr_heading_angle = heading_angle + (np.pi)

        # rotate the instance so that it is parallel to the scan axes
        rotation_matrix = np.array(
            [
                [np.cos(corr_heading_angle), -np.sin(corr_heading_angle), 0],
                [np.sin(corr_heading_angle), np.cos(corr_heading_angle), 0],
                [0, 0, 1],
            ]
        )
        rotated_points = np.dot(instance_points[:, :3], rotation_matrix)
        ...
        center = np.dot(center, rotation_matrix.T)  # rotate instance back

        # Bounding-Box with [x, y, z, dx, dy, dz, heading]
        bounding_box = np.hstack((center, dimensions, corr_heading_angle)).reshape(1, 7)
        return bounding_box
